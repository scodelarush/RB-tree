...
void Map<T1, T2>::remove(T1 key) {
	...
	iter--;
	if (*iter == temp->value) values.pop_back();
	else {
...
void Map<T1, T2>::print(Node<T1, T2>* node) {
	...
	if (node->parent == NULL)
		std::cout << node->key << " : " << node->value << " [" << color << "] is root" << '\n';
	else if (node->parent->left == node)
	{
		std::cout << node->key << " : " << node->value << " [" << color << "] is " << node->parent->key << "left child" << '\n';
	}
	else
	{
		std::cout << node->key << " : " << node->value << " [" << color << "] is " << node->parent->key << "right child" << '\n';
	}
	print(node->left);
	print(node->right);
}
